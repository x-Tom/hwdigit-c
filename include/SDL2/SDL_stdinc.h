...
/* NOTE: these float-evaluate their arguments, so you should never have side effects in the parameters */
...
extern DECLSPEC float SDLCALL SDL_atan2(float y, float x);
...
extern DECLSPEC float SDLCALL SDL_trunc(float x);
...
extern DECLSPEC float SDLCALL SDL_round(float x);
extern DECLSPEC float SDLCALL SDL_roundf(float x);
extern DECLSPEC long SDLCALL SDL_lround(float x);
